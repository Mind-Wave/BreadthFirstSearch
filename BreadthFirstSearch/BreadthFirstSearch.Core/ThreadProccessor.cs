...
namespace BreadthFirstSearch.Core
{
    public class ThreadProccessor
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ThreadProccessor"/> class.
        /// </summary>
        /// <param name="parameterizedUserMethod">The parameterized user method.</param>
        public ThreadProccessor(ParameterizedThreadStart parameterizedUserMethod) : this()
        {
            _parameterizedUserMethod = parameterizedUserMethod;
        }

        private ThreadProccessor()
        {
        ...
        public void Start(object userData = null)
        {
            ...
            if (_thread == null)
            {
                _thread = new Thread(ThreadProccessorMethod) { IsBackground = true, Name = Guid.NewGuid().ToString() };
                _thread.Start();
            }
        ...
        private void ThreadProccessorMethod()
        {
            try
            {
                while (true)
                {
                    try
                    {
                        ...
                        {
                            _parameterizedUserMethod.Invoke(searchData);
                        }
                    }
